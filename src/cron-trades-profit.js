...
client.getOpenTrades(accountId, function(error, trades)
{
  Object.keys(trades).forEach(function(key) 
  {
    var tradeLine = trades[key];
    client.getPrice(tradeLine.instrument, accountId, function(error, ratesprices)
      {
        //console.log(ratesprices)
        if(error==null)
        {
          var currentPrice = ratesprices.bids[0].price;
          var tradeUnit = tradeLine.currentUnits;
          var tradePrice = tradeLine.price;
          var tradeId = tradeLine.id;
          //console.log(currentPrice+' - '+tradePrice+' - '+tradeUnit);
          var profit = (parseFloat(currentPrice) - parseFloat(tradePrice)) * parseFloat(tradeUnit);
          getGlobalConf("BUY",tradeLine.instrument,function(globalConf)
          {
            ...
            /*  console.log(currentPrice);
              console.log(czzzzountDecimals(currentPrice));
               ...
               var takeProfitPrice = currentPrice;
               ...
               //Stop loss based on instrument configuration     
                /*BEGIN STOP LOSS*/
           /*   if(tradeLine.instrument=="XAU_USD")
               var stopLossPrice = currentPrice/*restFloat(currentPrice,0.010)*/;
             /*  else
               var stopLossPrice = /*restFloat(currentPrice,0.00008)*/;
               var stopLossPrice = currentPrice

               if(profit < (globalConf.stopLoss*(-1)))
               {
                console.log('prepare stop loss')
                 if(globalConf.alreadyInvested == 1)
                 {
                   if(globalConf.enabled)
                   {
                    ...
                    if(parseFloat(currentPrice) > parseFloat(globalConf.minPrice) && parseFloat(currentPrice) < parseFloat(globalConf.maxPrice))                  
                    {
                      ...
                      if(globalConf.enabled == 1)
                      {
                        ...
                        {   
                         ...
                         createTrade(tradeLine.instrument, globalConf.maxUnits, globalConf.id);
                        }
                      }
                    }  
                  }    
    ...
    setGlobalConf(confId,{alreadyInvested:1},function(err, result)
    {
      if(result.affectedRows>0)
      {
        client.createOrder(accountID,{"order":{"units":units, "instrument": instrument, "timeInForce":"FOK", "type": "MARKET", "positionFill": "DEFAULT"}},function(result)
        {
          ...
          sendNotification("Trade Order Requested","Trade ID: "+tradeId +"\n"+"Units: "+units+"\n"+"Date: "+nowDateTime);     
        });
      }
    }); 
  }    
